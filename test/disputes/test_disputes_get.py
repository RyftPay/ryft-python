...
@pytest.mark.asyncio
async def test_disputes_get(disputes_client, mock_ryft_client):
    mock_ryft_client.get.return_value = mock_dispute_resp()
    resp = await disputes_client.get(mock_dispute_id)
    mock_ryft_client.get.assert_called_once_with(
        f"disputes/{mock_dispute_id}",
    )
    assert resp == mock_dispute_resp()


@pytest.mark.asyncio
async def test_disputes_get_error(disputes_client, mock_ryft_client):
    mock_ryft_client.get.side_effect = mock_ryft_error()
    with pytest.raises(RyftError):
        await disputes_client.get(mock_dispute_id)
