...
@pytest.mark.asyncio
async def test_disputes_challenge(disputes_client, mock_ryft_client):
    mock_ryft_client.post.return_value = mock_dispute_resp()
    resp = await disputes_client.challenge(mock_dispute_id)
    mock_ryft_client.post.assert_called_once_with(
        f"disputes/{mock_dispute_id}/challenge", {}
    )
    assert resp == mock_dispute_resp()


@pytest.mark.asyncio
async def test_disputes_challenge_error(disputes_client, mock_ryft_client):
    mock_ryft_client.post.side_effect = mock_ryft_error()
    with pytest.raises(RyftError):
        await disputes_client.challenge(mock_dispute_id)
