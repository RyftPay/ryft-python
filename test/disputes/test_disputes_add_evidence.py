...
from test.disputes.mock_data.mock_add_dispute_evidence_req import (
    mock_add_dispute_evidence_req,
)
...
@pytest.mark.asyncio
async def test_disputes_add_evidence(disputes_client, mock_ryft_client):
    mock_ryft_client.patch.return_value = mock_dispute_resp()
    req = cast(AddDisputeEvidenceRequest, mock_add_dispute_evidence_req())
    resp = await disputes_client.add_evidence(mock_dispute_id, req)
    mock_ryft_client.patch.assert_called_once_with(
        f"disputes/{mock_dispute_id}/evidence", mock_add_dispute_evidence_req()
    )
...
async def test_disputes_add_evidence_error(disputes_client, mock_ryft_client):
    ...
    with pytest.raises(RyftError):
        req = cast(DeleteDisputeEvidenceRequest, mock_add_dispute_evidence_req())
        await disputes_client.add_evidence(mock_dispute_id, req)
