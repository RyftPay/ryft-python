...
@pytest.mark.asyncio
async def test_payment_sessions_list_transactions_custom_params(
    payment_sessions_client, mock_ryft_client
):
    mock_ryft_client.get.return_value = mock_payment_sessions_txs_resp()
    resp = await payment_sessions_client.list_transactions(
        mock_payment_session_id,
        ascending=True,
        limit=10,
        startsAfter="foo",
        account=mock_account_id,
...
@pytest.mark.asyncio
async def test_payment_sessions_list_transactions_error(
    payment_sessions_client, mock_ryft_client
):
    mock_ryft_client.get.side_effect = mock_ryft_error()
    with pytest.raises(RyftError):
        await payment_sessions_client.list_transactions(
            mock_payment_session_id, account=mock_account_id
        )
