...
@pytest.mark.asyncio
async def test_payment_sessions_void(payment_sessions_client, mock_ryft_client):
    mock_ryft_client.post.return_value = mock_payment_sessions_txs_resp()
    resp = await payment_sessions_client.void(
        mock_payment_session_id, account=mock_account_id
    )
    mock_ryft_client.post.assert_called_once_with(
        f"payment-sessions/{mock_payment_session_id}/voids", {}, account=mock_account_id
    )
    assert resp == mock_payment_sessions_txs_resp()


@pytest.mark.asyncio
async def test_payment_sessions_void_error(payment_sessions_client, mock_ryft_client):
    mock_ryft_client.post.side_effect = mock_ryft_error()
    with pytest.raises(RyftError):
        await payment_sessions_client.void(mock_payment_session_id)
