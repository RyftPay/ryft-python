...
@pytest.mark.asyncio
async def test_subscriptions_get(subscriptions_client, mock_ryft_client):
    mock_ryft_client.get.return_value = mock_subscription_resp()
    resp = await subscriptions_client.get(mock_subscription_id)
...
@pytest.mark.asyncio
async def test_subscriptions_get_error(subscriptions_client, mock_ryft_client):
    mock_ryft_client.get.side_effect = mock_ryft_error()
    with pytest.raises(RyftError):
        await subscriptions_client.get(mock_subscription_id)
