...
@pytest.mark.asyncio
async def test_subscriptions_pause(subscriptions_client, mock_ryft_client):
    mock_ryft_client.patch.return_value = mock_subscription_resp()
    resp = await subscriptions_client.pause(mock_subscription_id)
    mock_ryft_client.patch.assert_called_once_with(
        f"subscriptions/{mock_subscription_id}/pause", None
    )
...
async def test_subscriptions_create_error(subscriptions_client, mock_ryft_client):
    ...
    with pytest.raises(RyftError):
        await subscriptions_client.pause(mock_subscription_id)
